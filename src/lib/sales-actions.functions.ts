...
async function loadCompanyContext(ctx: Ctx) {
  const [{ data: settings }, { data: services }, { data: objections }] = await Promise.all([
    ((ctx.supabase as any) as any)((supabase as any).from('company_settings')).select('*').limit(1).maybeSingle(),
    ((ctx.supabase as any) as any)((supabase as any).from('services')).select('id, name, description, price, unit, term, max_discount').eq('active', true),
    ((ctx.supabase as any) as any)((supabase as any).from('objections')).select('trigger, response').limit(10),
  ])
...
export const draftInitialContact = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: lead, error } = await ((ctx.supabase as any) as any)((supabase as any).from('leads')).select('*').eq('id', data.lead_id).maybeSingle()
...
async function nextProposalNumber(ctx: Ctx): Promise<string> {
  ...
  const { count } = await ((ctx.supabase as any) as any)((supabase as any).from('proposals')).select('id', { count: 'exact', head: true })
...
export const autoDraftProposal = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: lead }, { data: qual }] = await Promise.all([
      ((ctx.supabase as any) as any)((supabase as any).from('leads')).select('*').eq('id', data.lead_id).maybeSingle(),
...
Resumo: ${qual?.summary ?? '—'}`
    ...
    if (cur >= 0 && cur < order.indexOf('Proposta')) {
      await ((ctx.supabase as any) as any)((supabase as any).from('leads')).update({ stage: 'Proposta' } as never).eq('id', data.lead_id)
    }
