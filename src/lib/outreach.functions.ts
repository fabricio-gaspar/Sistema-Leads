...
export async function isAnyContactSuppressed(
  ...
  const { data, error } = await (context.supabase as any).rpc('has_contact_suppression', {
    _lead_id: leadId,
    _hashes: hashes,
  })
  if (error) {
    const { data: rows, error: fallbackError } = await (context.supabase as any).from('contact_suppressions')
      .select('contact_hash')
...
export async function suppressLeadContactsInternal(ctx: Ctx, leadId: string) {
  ...
  if (rows.length) {
    await (context.supabase as any).from('contact_suppressions').upsert(rows as never, {
...
async function unsuppressLeadContactsInternal(ctx: Ctx, leadId: string) {
  ...
  const { error } = await (context.supabase as any).rpc('clear_contact_suppressions', {
    _lead_id: leadId,
    _hashes: hashes,
  })
...
async function loadLead(ctx: Ctx, leadId: string) {
  const { data, error } = await (context.supabase as any).from('leads').select('*').eq('id', leadId).maybeSingle()
  if (error) throw new Error(error.message)
...
export async function assertSandboxAllowed(
  ...
  const { data: settings } = await (context.supabase as any).from('company_settings')
    .select('sandbox_mode')
    .limit(1)
    .maybeSingle()
  if (!settings?.sandbox_mode) return { allowed: true }
  const { data: rows } = await (context.supabase as any).from('contact_points')
    .select('id')
    .eq('lead_id', leadId)
    .eq('sandbox', true)
    .limit(1)
...
async function updateChannelStatus(
  ...
  await (context.supabase as any).from('leads').update(patch as never).eq('id', leadId)
}
...
export async function enqueueOutreachTimeoutInternal(
  ...
  const { error } = await (context.supabase as any).from('outreach_jobs').insert({
...
async function tryStep(ctx: Ctx, lead: any, step: SequenceStep): Promise<void> {
  ...
  const enrollment = await getEnrollmentInternal((context.supabase as any), lead.id)
  let attemptQuery = (context.supabase as any).from('lead_outreach')
  ...
  await patchEnrollmentInternal((context.supabase as any), lead.id, {
  ...
  if (attempt > maxAttempts) {
    ...
    await patchEnrollmentInternal((context.supabase as any), lead.id, { last_error: 'max_attempts_reached' })
  ...
  const { data: row, error: rowError } = await (context.supabase as any).from('lead_outreach')
    .insert({
      lead_id: lead.id,
  ...
  if (channel === 'whatsapp') {
    ...
    if (result.ok) {
      ...
      await (context.supabase as any).from('lead_outreach')
        .update({
          status: 'sent',
          sent_at: now,
          provider: 'zapi',
        ...
        .eq('id', row.id)
      await (context.supabase as any).from('lead_messages').insert({
        lead_id: lead.id,
        sender: 'ia',
        sender_name: 'Ana (IA)',
        type: 'ia',
      ...
      await patchEnrollmentInternal((context.supabase as any), lead.id, { next_run_at: runAt, last_error: null })
    ...
    await patchEnrollmentInternal((context.supabase as any), lead.id, { last_error: result.error ?? 'whatsapp_failed' })
  ...
  if (channel === 'email') {
    ...
    await patchEnrollmentInternal((context.supabase as any), lead.id, { last_error: result.error ?? 'email_failed' })
  ...
  await (context.supabase as any).from('lead_outreach')
    ...
    .eq('id', row.id)
  await (context.supabase as any).from('lead_tasks').insert({
    lead_id: lead.id,
  ...
  await completeEnrollmentInternal((context.supabase as any), lead.id, 'phone_task_created')
...
async function advanceOrFinish(
  ...
  if (!stepAllowsContinue(failedStep, outcome)) {
    await (context.supabase as any).from('leads')
      .update({ next_action_at: null } as never)
      .eq('id', leadId)
    await patchEnrollmentInternal((context.supabase as any), leadId, { next_run_at: null, last_error: `stop_on_${outcome}` })
    await pauseEnrollmentInternal((context.supabase as any), leadId, `stop_on_${outcome}`)
    return
  }
  const lead = await loadLead(ctx, leadId)
  const bundle = await loadDefaultSequenceInternal((context.supabase as any))
  if (!bundle) {
...
async function finishNoMore(ctx: Ctx, lead: any, reason: string) {
  await (context.supabase as any).from('leads')
    ...
    .eq('id', lead.id)
  await completeEnrollmentInternal((context.supabase as any), lead.id, reason)
  await patchEnrollmentInternal((context.supabase as any), lead.id, { next_run_at: null, last_error: reason })
...
async function recordAiOutbound(
  ...
  const now = new Date().toISOString()
  const { count } = await (context.supabase as any).from('lead_outreach')
    .select('id', { count: 'exact', head: true })
    .eq('lead_id', lead.id)
    .eq('channel', channel)
  await (context.supabase as any).from('lead_outreach').insert({
    lead_id: lead.id,
    owner_id: lead.assigned_to || lead.owner_id || ctx.userId,
    channel,
...
async function registerUnanswered(ctx: Ctx, text: string) {
  ...
  const { data: existing } = await (context.supabase as any).from('unanswered_questions')
    .select('id, count')
    .eq('text', normalized)
    .maybeSingle()
  if (existing?.id) {
    await (context.supabase as any).from('unanswered_questions')
      .update({ count: (existing.count ?? 1) + 1, resolved: false } as never)
      .eq('id', existing.id)
    return
  }
  await (context.supabase as any).from('unanswered_questions').insert({
    text: normalized,
    count: 1,
    resolved: false,
  } as never)
...
async function handoffInbound(
  ...
  const { data: existingTasks } = await (context.supabase as any).from('lead_tasks')
    .select('id')
    .eq('lead_id', lead.id)
    .eq('completed', false)
  ...
  await (context.supabase as any).from('leads').update(leadPatch as never).eq('id', lead.id)
  await pauseEnrollmentInternal((context.supabase as any), lead.id, `handoff:${rule.category}`)
...
export async function handleInboundWithAiInternal(
  ...
  const [{ data: settings }, { data: services }, { data: objections }, knowledgeChunks, { data: learnedAnswers }, { data: history }] = await Promise.all([
    (context.supabase as any).from('company_settings')
      ...
      .limit(1)
      .maybeSingle(),
    (context.supabase as any).from('services').select('name, description, price, unit, term').eq('active', true).order('name'),
    (context.supabase as any).from('objections').select('trigger, response').order('created_at', { ascending: false }).limit(30),
    loadKnowledgeSnippetInternal((context.supabase as any), 8000),
    (context.supabase as any).from('unanswered_questions')
      .select('text, answer')
      .eq('resolved', true)
      .not('answer', 'is', null)
      .limit(50),
    (context.supabase as any).from('lead_messages')
      .select('sender, text')
      .eq('lead_id', leadId)
...
Base aprovada: ${knowledge}`
  ...
  try {
    ...
    if (decision.qualification) {
      ...
      const { data: previousQualification } = await (context.supabase as any).from('lead_qualifications')
        .select('*')
        .eq('lead_id', lead.id)
        .maybeSingle()
      ...
      const { error: qualificationError } = await (context.supabase as any).from('lead_qualifications').upsert({
        lead_id: lead.id,
    ...
    await (context.supabase as any).from('leads').update({ last_contact: new Date().toISOString() } as never).eq('id', lead.id)
...
export const startOutreach = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    let enrollment = await getEnrollmentInternal((context.supabase as any), lead.id)
    if (!data.restart && enrollment?.status === 'active' && enrollment.last_step_at) {
      return { ok: false, reason: 'cadence_already_active' }
    }
    if (!enrollment) {
      enrollment = await ensureEnrollmentInternal((context.supabase as any), lead.id)
      ...
      await resumeEnrollmentInternal((context.supabase as any), lead.id)
    ...
    if (!target) {
      ...
      await pauseEnrollmentInternal((context.supabase as any), lead.id, 'no_channel_available')
...
export const pauseAi = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.paused) {
      ...
      const { error: closeError } = await (context.supabase as any).from('lead_handoffs')
...
export const sendManualWhatsapp = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error: outreachError } = await (context.supabase as any).from('lead_outreach').insert({
      lead_id: lead.id,
      owner_id: ctx.userId,
      channel: 'whatsapp',
      status: result.ok ? 'sent' : 'failed',
    ...
    const { error: messageError } = await (context.supabase as any).from('lead_messages').insert({
      lead_id: lead.id,
      sender: 'human',
...
export const listOutreach = createServerFn({ method: 'POST' })
  ...
  .inputValidator((d: unknown) => z.object({ lead_id: z.string().uuid() }).parse(d))
  .handler(async ({ data, context }) => {
    const { data: rows, error } = await (context.supabase as any).from('lead_outreach')
      .select('*')
      .eq('lead_id', data.lead_id)
...
export async function triggerOutreachInternal(ctx: Ctx, leadId: string) {
  ...
  const bundle = await loadDefaultSequenceInternal((context.supabase as any))
  if (!bundle) return
  const enrollment = await getEnrollmentInternal((context.supabase as any), leadId)
  if (enrollment && (enrollment.status === 'paused' || enrollment.status === 'cancelled' || enrollment.status === 'completed')) return
  if (!enrollment) await ensureEnrollmentInternal((context.supabase as any), leadId)
