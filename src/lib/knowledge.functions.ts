...
export const getKnowledgeStats = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ count: activeChunks }, { count: staleChunks }, { count: docsWithText }] = await Promise.all([
      ((((context as any).supabase as any) as any)).from('knowledge_chunks' as any) as any.select('id', { count: 'exact', head: true }).eq('status', 'active'),
      ((((context as any).supabase as any) as any)).from('knowledge_chunks' as any) as any.select('id', { count: 'exact', head: true }).eq('status', 'stale'),
