...
export const getKnowledgeStats = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ count: activeChunks }, { count: staleChunks }, { count: docsWithText }] = await Promise.all([
      ((context.supabase as any) as any).from('knowledge_chunks').select('id', { count: 'exact', head: true }).eq('status', 'active'),
      ((context.supabase as any) as any).from('knowledge_chunks').select('id', { count: 'exact', head: true }).eq('status', 'stale'),
      context.supabase
        .from('documents')
