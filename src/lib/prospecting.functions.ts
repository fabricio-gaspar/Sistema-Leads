...
export const importExternalAsLead = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await ((context.supabase as any) as any).from('leads').insert(payload as never).select().single()
    if (error) throw new Error(error.message)

    await ((context.supabase as any) as any).from('audit_logs').insert({
