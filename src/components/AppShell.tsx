...
export function AppShell({
  ...
  const title =
    TITLES[pathname] ??
    (pathname.startsWith("/leads/") ? "Detalhe do Lead" : "Wayflex CRM");
      ...
      >
        <div className="flex h-14 items-center gap-2 px-5 border-b border-sidebar-border">
          ...
          <div className="leading-tight">
            <div className="text-sm font-semibold text-white">WAYFLEX</div>
...
function SearchResults({ res, onPick }: { res: SearchResData | undefined; onPick: () => void }) {
  ...
  const empty = res.leads.length === 0 && res.proposals.length === 0;
  if (empty) {
