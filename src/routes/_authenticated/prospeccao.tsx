...
import { Sparkles, Search, Loader2, Download, Plus, ExternalLink, RotateCcw, Info } from "lucide-react";
import { Card } from "@/components/ui-kit";
import { searchExternalCompanies, importExternalAsLead, type ExternalCompany } from "@/lib/prospecting.functions";
...
function Prospeccao() {
  ...
  function exportCSV() {
    ...
    downloadCSV(
      `prospeccao-externa-${new Date().toISOString().slice(0, 10)}.csv`,
      results.map((c) => ({
        cnpj: c.cnpj,
      ...
      <Card>
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <div className="text-[14px] font-semibold text-text-title">Prospecção externa — Receita Federal + IA</div>
            <div className="text-[12px] text-text-sec">
              Busca empresas ativas na base pública da Receita (via CNPJ.ws) e a IA <b>Ana</b> pontua o fit com o seu perfil de cliente ideal.
            </div>
          </div>
        </div>
        <div className="mt-3 flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-[12px] text-amber-900">
          ...
          <span>
            API pública gratuita limitada a <b>3 consultas por minuto</b>. Resultados ficam em cache por 7 dias. Para maior volume, gere uma chave grátis em <a href="https://cnpj.ws" target="_blank" rel="noreferrer" className="underline">cnpj.ws</a> e configure a secret <code>CNPJWS_API_KEY</code>.
          </span>
        </div>
      </Card>
        ...
        <Card padded={false}>
          <div className="flex items-center justify-between border-b border-border-card px-4 py-3">
            <div className="text-[13px] text-text-body">
              <b>{results.length}</b> empresas encontradas
          ...
          {results.length === 0 ? (
            <div className="p-10 text-center text-[13px] text-text-sec">
              Nenhuma empresa encontrada. Tente ampliar os filtros (remover CNPJ, ampliar UF).
            </div>
          ) : (
                ...
                {results.map((c) => (
                  <>
                    <tr key={c.cnpj} className="hover:bg-bg-general/40">
                      ...
                      <td className="p-3 align-top text-text-body">
                        <div>{c.telefone ?? "—"}</div>
                        <div className="text-[11px] text-text-ter">{c.email ?? ""}</div>
                      </td>
                      <td className="p-3 align-top">
                        {c.score != null ? (
                          <button
      ...
      {!applied && (
        <Card>
          <div className="text-center text-[13px] text-text-sec py-6">
            Preencha os filtros acima e clique em <b>Buscar na Receita</b> para começar a prospectar.
          </div>
        </Card>
      )}
    </div>
